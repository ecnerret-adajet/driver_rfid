...
class ReportEntriesTransformer extends TransformerAbstract
{
    ...
    public function transform(GateEntry $gateEntry)
    {
        return [
            ...
            'sap_ts_in_to_sap_loading_start' => !empty($gateEntry->hasShipment->loading->ts_in) && !empty($gateEntry->hasShipment->loading->loading_start) ?  $this->DateTimeDiff(date('Y-m-d H:i', strtotime($gateEntry->hasShipment->loading->ts_in)),date('Y-m-d H:i', strtotime($gateEntry->hasShipment->loading->loading_start))) : "N/A",
            'sap_loading_start_to_sap_loading_end' => !empty($gateEntry->hasShipment->loading->loading_start) && !empty($gateEntry->hasShipment->loading->loading_end) ? $this->DateTimeDiff(date('Y-m-d H:i', strtotime($gateEntry->hasShipment->loading->loading_start)),date('Y-m-d H:i', strtotime($gateEntry->hasShipment->loading->loading_end))) : "N/A",
            'sap_loading_end_to_sap_ts_out' => !empty($gateEntry->hasShipment->loading->loading_end) &&  !empty($gateEntry->hasShipment->loading->ts_out) ? $this->DateTimeDiff(date('Y-m-d H:i', strtotime($gateEntry->hasShipment->loading->loading_end)),date('Y-m-d H:i', strtotime($gateEntry->hasShipment->loading->ts_out))) : "N/A",
            'sap_ts_out_to_plant_out' => !empty($gateEntry->hasShipment->loading->ts_out) && !empty($gateEntry->hasGateOut->LocalTime) ? $this->DateTimeDiff(date('Y-m-d H:i', strtotime($gateEntry->hasShipment->loading->ts_out)),date('Y-m-d H:i', strtotime($gateEntry->hasGateOut->LocalTime))) : "N/A",
